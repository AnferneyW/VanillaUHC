...
execute @s[tag=mainhost,scores={limitations=0}] ~ ~ ~ execute @a[tag=playing,scores={detect_iron=1}] ~ ~ ~ give @s iron_ingot 1
execute @s[tag=mainhost,scores={limitations=1}] ~ ~ ~ execute @a[tag=playing,scores={detect_iron=1,iron_limit=0..63}] ~ ~ ~ give @s iron_ingot 1
execute @a[tag=playing,scores={detect_iron=1}] ~ ~ ~ scoreboard players add @s iron_limit 1
...
execute @a[tag=playing,scores={detect_gold=1}] ~ ~ ~ tellraw @a[tag=vanished] {"rawtext":[{"translate":"spectate.notification.ore","with":{"rawtext":[{"selector":"@s"},{"text":"1 Gold"}]}}]}
